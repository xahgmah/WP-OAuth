...
function get_oauth_identity($wpoa) {
	...
	$oauth_identity['id'] = $result_obj['sub']; // PROVIDER SPECIFIC: Edx returns the user's OAuth identity as id
	$oauth_identity['user_email'] = $result_obj['email']; // PROVIDER SPECIFIC: Edx returns an array of email addresses. To respect privacy we currently don't collect the user's email address.
	$oauth_identity['username'] = $result_obj['name'];
